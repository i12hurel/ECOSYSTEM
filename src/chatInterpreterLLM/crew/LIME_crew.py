...
def return_resultado_crew_LIME(model, x_train, instancia, metadata):
  crew_LIME = Crew(
    agents=[return_explanation_LIME_agent(), return_report_LIME_agent()],
    tasks=[return_task_explanation_LIME(model, x_train, instancia), return_task_report_LIME(metadata)],
      verbose=True
  )

  resultado_LIME = crew_LIME.kickoff(inputs={"lime_output": explicacion_LIME(model, x_train, instancia), "metadata": metadata})
